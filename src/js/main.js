...
function initIconFallbacks() {
    ...
    setTimeout(() => {
        ...
        const loginButtons = document.querySelectorAll('.login-btn, .login-btn-mobile');
        loginButtons.forEach(button => {
            const iconifyIcon = button.querySelector('iconify-icon');
            const fallbackIcon = button.querySelector('.icon-fallback');
            
            if (iconifyIcon && fallbackIcon) {
                // Check if iconify icon is visible and has content
                if (!iconifyIcon.innerHTML || iconifyIcon.innerHTML.trim() === '' || 
                    iconifyIcon.offsetWidth === 0 || iconifyIcon.offsetHeight === 0) {
                    console.log('Login button icon not loaded, showing fallback');
                    iconifyIcon.style.display = 'none';
    ...
    // Additional check after 3 seconds for any remaining issues
    setTimeout(() => {
        const loginButtons = document.querySelectorAll('.login-btn, .login-btn-mobile');
        loginButtons.forEach(button => {
            const iconifyIcon = button.querySelector('iconify-icon');
            ...
            if (iconifyIcon && fallbackIcon && iconifyIcon.style.display !== 'none') {
                // If iconify icon is still not working, force fallback
                if (!iconifyIcon.innerHTML || iconifyIcon.innerHTML.trim() === '') {
                    console.log('Forcing fallback for login button icon');
                    iconifyIcon.style.display = 'none';
                    fallbackIcon.style.display = 'inline-block';
                }
            }
...
function initVideoCarousel() {
    ...
    function showSlide(slideIndex) {
        ...
        if (videoSlides[slideIndex]) {
            videoSlides[slideIndex].classList.add('active');
        }
        if (heroContents[slideIndex]) {
            heroContents[slideIndex].classList.add('active');
        }
